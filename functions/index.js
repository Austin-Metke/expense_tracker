...
exports.deleteData = functions
    ...
    .onDelete(async (user, context) => {
        ...
        //Recursively deletes all documents and sub-collections under user collection
        await admin
            .firestore()
        ...
        //Delete user stats document
        await admin.firestore().doc('stats/' + user.uid).delete();

        //Delete users archived receipts
        await admin.firestore().recursiveDelete(admin.firestore().collection('archivedReceipts').doc(user.uid)).then(
            () => {
                console.log("Archived receipts for user " + user.uid + "successfully deleted!");
            }
        ).catch((reason) => {
            console.log("An error occurred deleting archived receipts! " + reason.code);
        });
...
exports.updateUserExpenses = functions
    .region("us-west2")
    .firestore.document("users/{userID}/receipts/{receiptID}")
    .onWrite(async (change, context) => {
        ...
        if (!change.before.exists) {
            ...
            if (
                beforeExpenseType !== afterExpenseType &&
...
exports.archive = functions
    ...
    .onRun(async (context) => {
        ...
        for (let i = 0; i < usersQuerySnapshot.size; i++) {
            ...
            const receiptColReference = await firestore.collection(
                "users/" + usersDocReference.id + "/receipts",
            );
            ...
            for (let j = 0; j < receiptSnapshot.size; j++) {
                ...
                await firestore
                    .collection("archivedReceipts")
                    .doc(usersDocReference.id)
                    .collection("receipts")
                    .doc(receiptDocReference.id)
                    .set(receiptDocReference.data());
...
exports.updateCumulativeStats = functions
    .region("us-west2")
    .firestore.document("stats/{userID}")
    .onUpdate(async (change, context) => {
        ...
        if (change.before.get("receiptCount") > change.after.get("receiptCount")) {
            ...
            change.before.get("receiptCount") < change.after.get("receiptCount")
        ) {
            await statsRef.update({
                cumulativeReceiptCount: increment(1),
            });
        ...
        if (change.before.get("receiptTotal") > change.after.get("receiptTotal")) {
            let total =
                change.after.get("receiptTotal") - change.before.get("receiptTotal");

            await statsRef.update({
                cumulativeReceiptTotal: increment(total),
            });
        } else if (
            change.before.get("receiptTotal") < change.after.get("receiptTotal")
        ) {
            let total =
                change.after.get("receiptTotal") - change.before.get("receiptTotal");
            await statsRef.update({
        ...
        if (change.before.get("foodTotal") > change.after.get("foodTotal")) {
            let total =
                change.after.get("foodTotal") - change.before.get("foodTotal");
        ...
        if (change.before.get("otherCount") > change.after.get("otherCount")) {
            ...
            change.before.get("otherCount") < change.after.get("otherCount")
        ) {
            await statsRef.update({
                cumulativeOtherCount: increment(1),
            });
        ...
        if (change.before.get("otherTotal") > change.after.get("otherTotal")) {
            let total =
                change.after.get("otherTotal") - change.before.get("otherTotal");

            await statsRef.update({
                cumulativeOtherTotal: increment(total),
            });
        } else if (
            change.before.get("otherTotal") < change.after.get("otherTotal")
        ) {
            let total =
                change.after.get("otherTotal") - change.before.get("otherTotal");
        ...
        if (change.before.get("toolsCount") > change.after.get("toolsCount")) {
            ...
            change.before.get("toolsCount") < change.after.get("toolsCount")
        ) {
            await statsRef.update({
                cumulativeToolsCount: increment(1),
            });
        ...
        if (change.before.get("toolsTotal") > change.after.get("toolsTotal")) {
            let total =
                change.after.get("toolsTotal") - change.before.get("toolsTotal");

            await statsRef.update({
                cumulativeToolsTotal: increment(total),
            });
        } else if (
            change.before.get("toolsTotal") < change.after.get("toolsTotal")
        ) {
            let total =
                change.after.get("toolsTotal") - change.before.get("toolsTotal");
        ...
        if (change.before.get("travelCount") > change.after.get("travelCount")) {
            ...
            change.before.get("travelCount") < change.after.get("travelCount")
        ) {
            await statsRef.update({
                cumulativeTravelCount: increment(1),
            });
        ...
        if (change.before.get("travelTotal") > change.after.get("travelTotal")) {
            let total =
                change.after.get("travelTotal") - change.before.get("travelTotal");

            await statsRef.update({
                cumulativeTravelTotal: increment(total),
            });
        } else if (
            change.before.get("travelTotal") < change.after.get("travelTotal")
        ) {
            let total =
                change.after.get("travelTotal") - change.before.get("travelTotal");
